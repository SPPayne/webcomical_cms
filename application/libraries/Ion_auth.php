      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				else
				{
					...
					//Original ion_auth code below:
					//echo $message;die; //DEBUG
					//$this->email->clear();
					...
					/* It's story time, kids! I had some real difficulty sending emails without a real "from" address.
					 * However, WordPress doesn't have any problems like that, so after investigation it turns out that 
					 * they use PHPMailer. So, in order to fudge the best possible setup for sending regardless of 
					 * config, I've integrated PHPMailer.
					 * Yaaaay!
					 */
					...
					try {
						...
						//Fetch webcomic config
						$this->load->config('webcomic',TRUE);
						
						//Fetch site data...
						$this->load->model('Settings_model','Settings');
						$data['settings'] = $this->Settings->fetch_site_values();
						
						//Set from email and from name
						$from_address = strtolower($this->config->item('app_name','webcomic')) . "@" . trim(preg_replace('#^https?://#','',base_url()),'/');
						$from_name	  = $this->config->item('app_name','webcomic') . " (" . $data['settings']['site_name'] . ")";
						
						//DEBUG
						//echo $from_address . " - " . $from_name;die;
						...
						$mail->Subject = $this->config->item('app_name','webcomic') . ' Password Reset';
						$mail->Body    = $message;
					...
					//Epic fail
						...
						//echo 'Message could not be sent. Mailer Error: ', $mail->ErrorInfo; //PHPMailer error
						$this->set_error('forgot_password_unsuccessful');
						return FALSE;
						
					}
					
					//Orig ion_auth code below:
					/*if ($this->email->send())
					{
						$this->set_message('forgot_password_successful');
						return TRUE;
