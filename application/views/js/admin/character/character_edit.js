$(document).ready(function(){
	...
	$('.btn-file :file').on('fileselect', function(event, numFiles, label){
		...
		var img = $('input[name=userfile]');
		img = img[0].files[0]; //Get from object
		formData.append('userfile',img); //Append to formdata
		
		$.ajax({
			type			: "POST",
			url				: base_url+"admin/upload_profile_img/"+characterid,
			data			: formData,
			contentType		: false,
			processData		: false,
	...
	//Submit form
	$('#upload_character').validator().on('submit', function(e){
		...
		if(!e.isDefaultPrevented()){
			...
			$.ajax({
				...
				url         : base_url+'admin/update_character/'+characterid,
