...
$(document).ready(function(){
	...
	//Move page
	$('#characters-table').on("click",'a[class^=charactermove-]',function(event){
		...
		$.ajax({
			...
			type        : "POST",
			url         : url,
			success		: function(response){ move_character(response,this); },
	...
	function move_character(response,context){
	...
	//Delete a character
	$('#characters-table').on("click",'.delete-button',function(event){
	...
	$('#confirm_deletion').click(function(){
		...
		$.ajax({
			...
			type        : "POST",
			url         : deletion_url,
			success		: function(response){ delete_character(response,delete_context); },
			error		: function(response){ handle_error('Server could not complete the request. Please check that the website is available and try again.'); }
		});
	...
	//Handle AJAX response
	function delete_character(response,context){
