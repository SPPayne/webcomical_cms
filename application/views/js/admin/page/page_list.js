...
$(document).ready(function(){
	...
	//Move page
	$('#pages-table').on("click",'a[class^=pagemove-]',function(event){
		...
		$.ajax({
			...
			type        : "POST",
			url         : url,
			success		: function(response){ move_page(response,this); },
	...
	function move_page(response,context){
	...
	//Delete a page
	$('#pages-table').on("click",'.delete-button',function(event){
	...
	$('#confirm_deletion').click(function(){
		...
		$.ajax({
			...
			type        : "POST",
			url         : deletion_url,
			success		: function(response){ delete_page(response,delete_context); },
			error		: function(response){ handle_error('Server could not complete the request. Please check that the website is available and try again.'); }
		});
	...
	//Handle AJAX response
	function delete_page(response,context){
